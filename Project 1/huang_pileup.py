...
def snp_calls(ref_string, consensus_string, start_index):
    ...
    for i in range(len(ref_string)):
        if ref_string[i] != consensus_string[i]: # and ref_string[i+1] == consensus_string[i+1] and ref_string[i-1] == consensus_string[i-1]
            snps.append([ref_string[i], consensus_string[i], start_index + i])
    return snps

def remove_ins(f_snps):
    global snps
    """
    param f_snps: list of snps
    do: remove potential fake snps (in-del in nature)
    no return
    """
    temp_snps = []
    i = 0
    for i in range(len(f_snps)):
        if i==0:
            if (f_snps[i+1][2]-f_snps[i][2])>=4:
                temp_snps.append(f_snps[i])
        elif i>=len(f_snps)-1:
            if (f_snps[i][2]-f_snps[i-1][2])>=4:
                temp_snps.append(f_snps[i])
        else:
            if (f_snps[i+1][2]-f_snps[i][2])>=4 and (f_snps[i][2]-f_snps[i-1][2])>=4:
                temp_snps.append(f_snps[i])
...
if __name__ == "__main__":
    ...
    snps = remove_ins(snps) # not yet implemented
